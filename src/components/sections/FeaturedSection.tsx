...
import { BedDouble, Maximize2, ArrowLeft } from 'lucide-react';
...
export default function FeaturedSection() {
  ...
  useEffect(() => {
    fetch('/api/properties')
      .then(r => r.json())
      .then((data: Property[]) => {
        // Show explicitly featured listings first, then fill the remaining
        // slots with the most recent ones (API already orders by newest).
        const featured = data.filter(p => p.featured);
        const rest = data.filter(p => !p.featured);
        setProperties([...featured, ...rest].slice(0, 4));
      })
      .catch(() => setProperties([]));
  }, []);
  ...
  const Specs = ({ rooms, area }: { rooms: string; area: number }) => (
    <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
      {[
        ...(rooms !== '—' ? [{ Icon: BedDouble, val: rooms }] : []),
        { Icon: Maximize2, val: `${area}${sqmLabel}` },
      ].map(({ Icon, val }, j) => (
  ...
  return (
    <section style={{ background: t.altBg, direction: dir, padding: isMobile ? '60px 0' : '100px 0', borderTop: '1px solid rgba(217,186,160,0.1)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0 18px' : '0 32px' }}>
            ...
            <div key={p.id} style={{ ...cardBase, ...rv(visible, 0.1 + i * 0.12) }} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={() => router.push(`/properties/${p.slug}`)}>
              <div style={{ position: 'relative', overflow: 'hidden', height: isMobile ? 240 : 360, background: '#111' }}>
                {p.photos[0] && (
                  ...
                  <img src={p.photos[0]} alt={isAr ? p.titleAr : p.titleEn} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.7s cubic-bezier(0.22,1,0.36,1)' }} />
                )}
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.5) 0%, transparent 55%)' }} />
                <LocationBadge location={isAr ? p.locationAr : p.locationEn} />
                {p.badge && <TagBadge tag={p.badge} />}
              </div>
              <div style={{ padding: '22px 22px 20px' }}>
                ...
                <Specs rooms={p.rooms} area={p.area} />
                <PriceRow price={formatPrice(parseUSD(p.price))} />
              </div>
            </div>
          ))}
        </div>
        ...
        {/* View all link */}
        <div style={{ textAlign: 'center', marginBottom: row2.length > 0 ? 18 : 0 }}>
        ...
        {row2.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: isMobile || row2.length === 1 ? '1fr' : '1fr 1fr', gap: 18 }}>
            {row2.map((p, i) => (
              <div
                key={p.id}
              ...
              >
                ...
                <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flex: 1 }}>
                  <div>
                    ...
                    <h3 style={{ fontFamily: "'Marcellus', serif", color: t.txt, fontSize: '1rem', margin: '0 0 14px', lineHeight: 1.3 }}>{isAr ? p.titleAr : p.titleEn}</h3>
                    <div style={{ width: '100%', height: 1, background: t.border, marginBottom: 14 }} />
                    <Specs rooms={p.rooms} area={p.area} />
                  </div>
                  <div style={{ fontFamily: "'Marcellus', serif", color: t.gold, fontSize: '1.1rem', marginTop: 8 }}>{formatPrice(parseUSD(p.price))}</div>
                </div>
              </div>
            ))}
          </div>
